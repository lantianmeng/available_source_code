...
MyGLWidget::MyGLWidget(QWidget *parent) : QOpenGLWidget(parent)
{
    connect(&timer, &QTimer::timeout, [=]()
    {
        ...
        //{
        //    QImage image = allImages.dequeue();
        //    setImage(image);
        //    qDebug() << allImages.size();
        //}
        m_dataMutex.lock();
        if(!varShowImage.isNull())
        {
            setImage(varShowImage);
        }
        m_dataMutex.unlock();
...
void MyGLWidget::UpdateImage(const QImage &image)
{
    m_dataMutex.lock();
    //allImages.enqueue(image);
    varShowImage = image;
    m_dataMutex.unlock();
}
